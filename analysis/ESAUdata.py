...
def ESAUdata(path,desire=[0],channels=[0],N = 450e3):
    ...
    for both potential options if referring to older measurements. 
    ...
    #load calibration file for each channel recorded
    cal = np.empty((len(gen),len(channels)),dtype = float)
    if isFile0 == True:
